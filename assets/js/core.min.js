/* Ducky's Blog custom.js */
var duckyScript = document.createElement("script");
duckyScript.type = "text/javascript";
duckyScript.src = "/gh/duckyGg/typecho@master/assets/js/ducky/custom.js";
document.body.appendChild(duckyScript);
...
const handsome_util = {
    ...
    set_innner_html: function(a, b, c) {
        ...
        if (null == h)
            ...
            if (void 0 === c)
                return g[a + "_html"] = b,
                void window.setTimeout("set_innerHTML('" + a + "', global_lock_pool['" + a + "_html']);", 10);
            if (h != a)
                return g[a + "_html"] = b,
                void window.setTimeout("set_innerHTML('" + a + "', global_lock_pool['" + a + "_html'], " + c + ");", 10)
        }
        i = "",
        ...
        for (var k = 0; k < b.length; k++)
            d[k] = b[k].replace(/<script[\s\S]*$/gi, ""),
            j[k] = {
                text: "",
                src: ""
            },
            j[k].text = b[k].substr(d[k].length),
            j[k].src = j[k].text.substr(0, j[k].text.indexOf(">") + 1),
            j[k].src = j[k].src.match(/src\s*=\s*(\"([^\"]*)\"|\'([^\']*)\'|([^\s]*)[\s>])/i),
            j[k].src ? (j[k].src[2] ? j[k].src = j[k].src[2] : j[k].src[3] ? j[k].src = j[k].src[3] : j[k].src[4] ? j[k].src = j[k].src[4] : j[k].src = "",
            j[k].text = "") : (j[k].src = "",
            j[k].text = j[k].text.substr(j[k].text.indexOf(">") + 1),
            j[k].text = j[k].text.replace(/^\s*<\!--\s*/g, ""));
        var l;
        l = void 0 === c ? 0 : c;
        for (var m, n, o, k = 0; k < j.length; k++) {
            ...
            j[k].src ? (m.src = j[k].src,
            void 0 === f[m.src] ? (f[m.src] = !0,
            l += 2e3) : l += 10) : (m.text = j[k].text,
            l += 10),
            m.defer = !0,
            ...
            n += ".appendChild(global_script_pool['" + m.id + "']);\n",
            window.setTimeout(n, l),
            o = "document.getElementsByTagName('head').item(0)",
            o += ".removeChild(document.getElementById('" + m.id + "'));\n",
    ...
    getDeviceState: function(a) {
        var b;
        return window.getComputedStyle ? b = window.getComputedStyle(a).getPropertyValue("z-index") : a.currentStyle && (b = a.currentStyle["z-index"]),
        parseInt(b, 10)
    },
    getPrefersDarkModeState: function(a) {
        if ("" === handsome_UI.mode || a) {
            ...
            if ("auto" === LocalConst.THEME_DARK_MODE) {
                ...
                } else if ("auto" === LocalConst.THEME_DARK_MODE_VALUE)
                    b = window.matchMedia("(prefers-color-scheme: dark)").matches;
                ...
                b = "light" !== LocalConst.THEME_DARK_MODE;
            return handsome_UI.mode = b ? "dark" : "light",
            b
        }
    ...
    hasClosestByTag: function(a, b) {
        ...
        for (var c = a, d = !1; c && !d && !c.classList.contains("vditor-reset"); )
            0 === c.nodeName.indexOf(b) ? d = !0 : c = c.parentElement;
        return d && c
    },
...
var handsome_ajax = {
    ...
    user_common_op_ajax: function(a, b) {
        a.find(".user_op_submit").addClass("active"),
        a.find(".animate-spin").addClass("show inline"),
        $.ajax({
            type: "POST",
            ...
            success: function(c) {
                ...
                $.ajax({
                    ...
                    error: function() {
                        ...
                        handsome_ajax.op_error_action(a),
                        !1
                    },
                    success: function(c) {
                        c = $.parseHTML(c),
                        a.find(".user_op_submit").removeClass("active"),
                        a.find(".animate-spin").removeClass("show inline");
                        try {
                            ...
                            var g = "";
                            return e.notice && "success|notice|error".indexOf(e.noticeType) >= 0 && (g = $.parseJSON(e.notice),
                            $.cookie(d + "__typecho_notice", null, {
                                path: f
                            }),
                            ...
                            LocalConst.USER_LOGIN = "1",
                            !1) : "register" === b && ("success" === e.noticeType ? ($.get(LocalConst.BLOG_URL, function(a, b) {
                                a = $("#easyLogin", a).html(),
                                $("#easyLogin").html(a),
                                $.message({
                                    title: LocalConst.REGISTER_TITLE,
                            ...
                            !1) : (g || console.log("notice_content should not empty!"),
                            $.message({
                                title: LocalConst.REGISTER_TITLE,
    ...
    register_ajax: function() {
        $("#register_form").submit(function() {
            ...
            $("#navbar-register-user").focus(),
            handsome_ajax.op_error_action($(this)),
            !1) : "" === b ? ($.message({
                title: LocalConst.REGISTER_TITLE,
                message: LocalConst.REGISTER_MAIL_INFO,
                type: "warning"
            }),
            $("#navbar-register-mail").focus(),
            handsome_ajax.op_error_action($(this)),
            !1) : (handsome_ajax.user_common_op_ajax($(this), "register"),
            !1)
        })
    },
    login_ajax: function() {
        $("#login_form").submit(function() {
            ...
            handsome_ajax.op_error_action($(this)),
            !1) : (handsome_ajax.user_common_op_ajax($(this), "login"),
            !1)
        })
    },
...
const handsome_basics = {
    ...
    nav_handle: function() {
        ...
        if (h.i = LocalConst[x(c)],
        "2" !== h[m.call(null, 105)]) {
            for (var d = 0; d < a.length; d++)
                b += "%u" + a[d].toString(16);
            h.t = unescape(b)
        }
        b.length > 0 && -1 === window.location.href.indexOf(window.atob("L2FkbWluL3ByZXZpZXc")) && (h.d[0][x(handsome_basics.j)] = h.t),
        $(document).on("click", "[ui-toggle-class]", function(a) {
            ...
            $.each(c, function(a, b) {
                var f = d[d.length && e];
                $(f).toggleClass(c[a]),
                ".app-aside" === f && ($("#aside").hasClass("off-screen") ? $("#footer").addClass("hide") : $("#footer").removeClass("hide")),
                e++
            }),
            b.toggleClass("active"),
            $(".settings").removeClass("active"),
            $(".tocify-mobile-panel").removeClass("active")
        }),
        ...
        $(document).on("click", "[ui-nav] a .parent_name", function(a) {
            ...
            b.setAttribute("href", $(this).parent("a").attr("href")),
            document.body.appendChild(b),
            b.click()
        }),
        $(document).on("click", "[ui-nav] a,[ui-nav-fold] a", function(a) {
            !LocalConst.LEFT_LOCATION || $("#alllayout").hasClass("app-aside-dock") || $("#alllayout").hasClass("app-aside-folded") || ($(this).addClass("select"),
            handsome_UI.last_select_left_root = $(this)),
            $("#footer").removeClass("hide"),
            e && e.trigger("mouseleave.nav");
            var b = $(this);
            if (b.parent().siblings(".active").toggleClass("active"),
            b.next().is("ul") && b.parent().toggleClass("active") && a.preventDefault(),
            $(".app-aside-fix").length > 0 && $(window).width() > 768) {
                var c, b = $(a.target), d = $(window).height();
                b.is("a") && b.next().is(".child-nav") && (e = b.parent().parent()),
                $(".app-aside-folded").length && e && (c = e.position().top,
                c + e.height() >= d && e.css("bottom", "50px"),
                c + 150 > d && e.css("bottom", d - c - 50).css("top", "auto")),
        ...
        $(document).on("mouseenter", "[ui-nav] a", function(a) {
            ...
            $(".app-aside-fix").length && !($(window).width() < 768)) {
                var b, c = $(a.target), d = $(window).height();
                !c.is("a") && (c = c.closest("a")),
                c.next().is("ul") && (e = c.next(),
                $(".app-aside-folded").length && void 0 === c.attr("no-fold") && (c.parent().addClass("active"),
                e.appendTo(".app-aside"),
                b = c.parent().position().top,
                e.css("top", b),
                e.attr("ui-nav-fold", ""),
                b + e.height() > d && e.css("bottom", "50px"),
                b + 150 > d && e.css("bottom", d - b - 50).css("top", "auto"),
                e.on("mouseleave.nav", function(a) {
                    f || (e.appendTo(c.parent()),
                    e.off("mouseleave.nav").css("top", "auto").css("bottom", "auto"),
  ...
  , handsome_comment = {
    ...
    initAndPjax_Complete: function() {
        LocalConst.COMMENT_SYSTEM === LocalConst.COMMENT_SYSTEM_ROOT ? (this.loadCommentEmoji(),
        this.placeholderChange(),
        this.secretComment(),
        this.watchInputInComment()) : LocalConst.COMMENT_SYSTEM === LocalConst.COMMENT_SYSTEM_CHANGYAN && this.loadChangYan(),
        this.starComment(),
        LocalConst.IS_PAJX_COMMENT ? this.ajaxComment() : this.noAjaxComment();
        var a = null;
    ...
    loadChangYan: function() {
        ...
        window.cyan = void 0,
        1 == $("#SOHUCS").length && ($("#spin_comment_changyan").addClass("show inline"),
    ...
    ajaxComment: function() {
        function a() {
            $("#submit span:first-child").text(LocalConst.REPLY_TIME_MACHINE),
            $("#time_toolbar").addClass("hide"),
            $("#submit").addClass("padder-v-none"),
            $("#comment_form").addClass("time_reply_comment"),
            $(".comment-respond").hasClass("hide") && ($(".comment-respond").removeClass("hide"),
            $(".comment-respond").addClass("hide_guest"))
        }
        function b() {
            $("#comment_form").removeClass("time_reply_comment"),
            $("#submit").removeClass("padder-v-none"),
            $("#time_toolbar").removeClass("hide"),
            $("#submit span:first-child").text(LocalConst.SUBMIT_TIME_MACHINE),
            $(".comment-respond").hasClass("hide_guest") && $(".comment-respond").addClass("hide")
        }
        function c(a) {
        ...
        function() {
            $("#comments").on("click", f + " a", function() {
                $(".time-machine").length > 0 ? (k = $(this).parent().parent().parent().parent().parent().attr("id"),
                a()) : k = $(this).parent().parent().parent().parent().attr("id"),
                $(h).focus()
            }),
            $("#cancel-comment-reply-link").click(function() {
                $(".time-machine").length > 0 && b(),
                k = ""
            })
        }(),
        $("#comment_form").submit(function() {
            if ($(i).attr("disabled", !0).fadeTo("slow", .5),
            $("#comment_form").find("#author")[0]) {
                if ("" == $("#comment_form").find("#author").val())
                    ...
                    c("#error"),
                    !1;
                if ("" == $("#comment_form").find("#mail").val() && "1" === LocalConst.COMMENT_NEED_EMAIL)
                    return $.message({
                        title: LocalConst.COMMENT_TITLE,
                ...
                if (!a.test(f) && "1" === LocalConst.COMMENT_NEED_EMAIL)
                    return $.message({
                        title: LocalConst.COMMENT_TITLE,
                ...
                if ("1" === LocalConst.COMMENT_NEED_URL) {
                    if ("" == $("#url").val())
                        ...
                        c("#error"),
                        !1;
                    if (!$("#url").val().startsWith("http://") && !$("#url").val().startsWith("https://"))
                        return $.message({
                            title: LocalConst.COMMENT_TITLE,
            ...
            c("#error"),
            !1) : ($("#secret_comment_checkbox").is(":checked") && (m[1].value = "[secret] " + m[1].value + "\n[/secret]"),
            $(i).addClass("active"),
            ...
            $.ajax({
                ...
                success: function(a) {
                    function l(a) {
                        ...
                        if ($(g, a).length)
                            try {
                                -1 !== window.location.href.indexOf("?tag=") && (b += "<li>" + LocalConst.COMMENT_NOT_BELONG_CURRENT_TAG + "</li>"),
                                "" != $("#comments .current").text() && "1" != $("#comments .current").text() && (b += "<li>" + LocalConst.COMMENT_NOT_IN_FIRST_PAGE + "</li>"),
                                f || (b += "<li>" + LocalConst.COMMENT_NO_EMAIL + "</li>"),
                                b += "<li>" + LocalConst.COMMENT_PAGE_CACHED + "</li>"
                            } catch (a) {
                                console.log(a)
                            }
                        else
                            b += "<li>" + LocalConst.COMMENT_BLOCKED + "</li>";
                        return b += "</ul>"
                    }
                    $(i).removeClass("active"),
                    $("#spin").removeClass("show inline");
                    try {
                        ...
                        try {
                            if ($(".comment-list").length > 0) {
                                if ($(".comment-list").html().match(/id=\"?comment-\d+/g).join().match(/\d+/g).sort(function(a, b) {
                                    return a - b
                                }).pop() === j || "" == k && $(".page-navigator .prev").length > 0)
                                    ...
                                    k = "",
                                    navigator.clipboard && (navigator.clipboard.writeText($(h).val()),
                                    $(h).val("")),
                                    c("#error"),
                                    !1
                            }
                        } catch (a) {}
                        k ? (a = $("#comment-" + j, a).hide(),
                        $("#" + k).find(".comment-children").length <= 0 && ($(".time-machine").length > 0 ? $("#" + k + " .time-machine .panel-footer").append("<div class='comment-children text-muted'><ol class='comment-list'></ol></div>") : $("#" + k).append("<div class='comment-children list-unstyled m-l-xxl'><ol class='comment-list'></ol></div>")),
                        j && $("#" + k + " .comment-children .comment-list").prepend(a),
                        k = "") : (a = $("#comment-" + j, a).hide(),
                        $(".time-machine").length > 0 ? $(".comment-list").prepend(a) : ($(".comment-list").length || $(g).prepend('<h4 class="comments-title m-t-lg m-b special"> 0 条评论</h4><ol class="comment-list"></ol>'),
                        $(".comment-list").prepend(a))),
                        $("#comment-" + j).fadeIn(),
                        c("#success");
                        var n;
                        $(e).length && (n = parseInt($(e).text().match(/\d+/)),
                        $(e).html($(e).html().replace(n, n + 1))),
                        TypechoComment.cancelReply(),
                        $(h).val(""),
                        $(i).attr("disabled", !1).fadeTo("slow", 1),
                        j ? d.animate({
                        ...
                        $(".time-machine").length > 0 && b(),
                        handsome_content.comment_ajax_complete("comment-" + j)
                    } catch (a) {
                        console.log(a)
                    }
                }
    ...
    noAjaxComment: function() {
        $("#submit").bind("click", function() {
            ...
            $("#author").focus(),
            !1) : "" === b && "1" === LocalConst.COMMENT_NEED_EMAIL ? ($.message({
                title: LocalConst.COMMENT_TITLE,
                message: LocalConst.COMMENT_EMAIL_INFO,
                type: "warning"
            }),
    ...
    secretComment: function() {
        $("#secret_comment_checkbox").change(function() {
            $(this).is(":checked") ? ($("#comment").addClass("secret_comment_textarea"),
            $(this).attr("placeholder", "")) : $("#comment").removeClass("secret_comment_textarea")
        })
    },
        ...
        $("#comments").on("click", ".star_talk", function() {
            ...
            $.get(LocalConst.BLOG_URL, {
                ...
                if ("1" === b) {
                    ...
                    c = c || 0,
                    a.children(".star_count").text(parseInt(c) + 1),
                    $.message({
                        title: LocalConst.OPERATION_NOTICE,
                    ...
                    "2" === b ? (a.children("i").css("color", "red"),
                    $.message({
                        title: LocalConst.OPERATION_NOTICE,
  ...
  , hd_ui = {
    ...
    showDialog: function(a) {
        const b = "form" === a.type;
        var c = "";
        if (b && (c = "<form method='POST' role='form'>"),
        c += '<div class="modal-body">',
        b) {
            if (a.form)
                for (var d in a.form)
                    c += this.formItem(a.form[d])
        } else
            a.text && (c += a.text);
        c += "</div>",
        c += b ? a.confirmText ? '<div class="modal-footer"><button type="submit" class="btn btn-primary">' + a.confirmText + "</button> </div>" : "" : a.confirmText ? '<div class="modal-footer"><button type="button" data-dismiss="modal"  class="btn btn-primary">' + a.confirmText + "</button> </div>" : "",
        b && (c += "</form>");
        ...
        $("body").append(e),
        handsome_enhance.feather(),
        $("#js_modal_dialog").modal(),
        $("#js_modal_dialog").on("hidden.bs.modal", function(a) {
            $("#js_modal_dialog").remove()
        })
    },
    confirm: function(a) {
        $("#js_modal_dialog").length > 0 ? (hd_ui.dialog_again = !0,
        $("#js_modal_dialog").modal("hide"),
        setTimeout(function() {
            hd_ui.showDialog(a)
  ...
  , handsome_cross = {
    tag_options: {
        ...
        noMatchTemplate: function() {
            return '<span style="padding: 0 5px">No Tag Matched</span>'
        }
    },
    tribute: null,
    init: function() {
    ...
    initAndPjax_Complete: function() {
        $("#time_content").length > 0 && (this.enhance_editor(),
        this.initTribute(),
        this.click_show_popup_menu())
    },
    enhance_editor: function() {
        const a = document.getElementById("comment");
        a.addEventListener("input", function(b) {
            a.style.height = "72px",
            a.style.height = b.target.scrollHeight + "px"
        }),
        a.addEventListener("keyup", function(a) {
            8 === a.keyCode ? (this.style.height = "inherit",
    ...
    click_show_popup_menu: function() {
        ...
        $("#time_content").on("click", ".time_operation", function() {
            $(this).children(".dropdown-menu").length <= 0 && ($(this).append('            <ul class="dropdown-menu">                                          <li role="presentation">                                            <a role="menuitem" class="edit" tabindex="-1">' + LocalConst.EDIT + '</a>                                        </li>                                        <li role="presentation">                                            <a role="menuitem" class="delete" tabindex="-1">' + LocalConst.DELETE + "</a>                                        </li>                                      </ul>"),
            $(this).on("click", "a.edit", function(a) {
                ...
                handsome_cross.tribute && handsome_cross.tribute.attach(g),
                g.addEventListener("input", function(a) {
                    g.style.height = "72px",
                    g.style.height = a.target.scrollHeight + "px"
                }),
                ...
                $("textarea[name=text]", f).val(e).focus(),
                ...
                $("form", f).submit(function() {
                    const a = $(this).parents(".time_edit")
                      ...
                      , d = $("#" + b + "-body");
                    var e = {};
                    e.origin = d.data("comment").text,
                    e.request_url = window.location.href,
                    ...
                    return d.data("comment", f),
                    $.post(d[0].dataset.edit_action, e, function(a) {
                        d.html(a.comment.content),
                        hd_ui.highlight(d),
                        handsome_content.comment_ajax_complete(b)
                    }, "json"),
                    a.remove(),
                    d.show(),
                    c.show(),
        ...
        $("#time_content").on("click", ".time_tag_operation", function() {
            ...
            $(this).children(".dropdown-menu").css("top", $(this)[0].getBoundingClientRect().top + 15),
            $(this).children(".dropdown-menu").css("left", $(this)[0].getBoundingClientRect().left - 20),
            $(this).on("click", "a.edit", function(b) {
    ...
    getOnlineTagList: function(a) {
        this.tag_options.values = JSON.parse(LocalConst.TAG_LIST),
        a && a()
    },
    addTag: function(a) {},
    ...
    initVditor: function() {
        handsome_util.addStyle(LocalConst.PLUGIN_URL + "/Handsome/assets/css/editor.css", "vditor_custom_css"),
        handsome_util.addStyle(LocalConst.VDITOR_CDN + "/dist/index.css", "vditor_css"),
        handsome_util.addScript(LocalConst.VDITOR_CDN + "/dist/index.min.js", "vditor_method", function() {
  ...
  , handsome_enhance = {
    ...
    pjax_complete: function() {
        this.initAndPjax_Complete(),
        "water_fall" === LocalConst.THUMB_STYLE && this.initWaterFall()
    },
    ...
    startPost: function() {
        ...
        $("#star_post").on("click", function() {
            if ($(this).hasClass("like_button_active"))
                $.message({
                    title: LocalConst.OPERATION_NOTICE,
                    message: "您已点赞",
                    type: "info"
                ...
                const b = $(this);
                $("#like_label").text(parseInt($("#like_label").text()) + 1),
                $(this).addClass("like_button_active"),
                setTimeout(function() {
                ...
                $.get(LocalConst.BLOG_URL, {
                    ...
                    "1" === a ? window.LocalConst.USER_LOGIN ? $.message({
                        ...
                        message: LocalConst.STAR_SUCCESS,
                        type: "success"
                    }) : "2" === a ? (b.children(".like_label").text(parseInt(b.children(".like_label").text()) - 1),
                    $.message({
                        title: LocalConst.OPERATION_NOTICE,
                        message: LocalConst.STAR_REPEAT,
                        type: "info"
                    })) : "-1" === a && (b.children(".like_label").text(parseInt(b.children(".like_label").text()) - 1),
                    b.removeClass("like_button_active"),
                    $.message({
                        title: LocalConst.OPERATION_NOTICE,
    ...
    statisticPane: function() {
        $("#statistic_pane").on("click", function() {
            "true" !== $("#statistic_pane").attr("data-status") && (Number(window.innerWidth) < 767 && $("#statistic_pane ~ .dropdown-menu .info-icons").addClass("hide"),
            handsome_util.addScript(LocalConst.ECHART_CDN + "/echarts.min.js", "echarts_js", function() {
                $.getJSON(LocalConst.BLOG_URL + "?action=get_statistic&size=" + window.innerWidth, function(a) {
                    if (LocalConst.CALENDAR_GITHUB)
                        try {
                            $.getJSON("https://python-github-calendar-api.vercel.app/api?" + LocalConst.CALENDAR_GITHUB, function(b) {
                                for (var c = 0, d = !1, e = 0; e < b.contributions.length && !d; e++) {
                                    ...
                                    for (var g = 0; g < f.length; ) {
                                        ...
                                        if (Date.parse(h.date) === Date.parse(i) ? (a.post_calendar.series[c][1] += h.count,
                                        c++,
                                        g++) : Date.parse(h.date) < Date.parse(i) ? g++ : c++,
                                        c >= a.post_calendar.series.length) {
                                            d = !0;
                                            break
                                        }
                                    }
    ...
    showTheEChart: function(a) {
        $(".loading-echart").addClass("hide"),
        $(".top-echart").removeClass("hide");
        ...
        var h, i, j, k;
        "light" === b ? (h = ["#ebedf0", "#c6e48b", "#7bc96f", "#239a3b", "#196127"],
        i = "#fff",
        j = "#3C4858",
        k = "#f9f9f9") : (h = ["#ebedf0", "#c6e48b", "#7bc96f", "#239a3b", "#196127"],
        i = "#000",
        j = "#fff",
        k = "#212121",
        LocalConst.IS_TRANSPARENT && (i = "#999",
        k = "transparent"));
        const l = "zh_CN" == LocalConst.RENDER_LANG || "zh_TW" == LocalConst.RENDER_LANG ? "cn" : "en"
          , m = {
            backgroundColor: k,
            tooltip: {
                padding: 10,
    ...
    plusFont: function() {
        $(".plus-font-size").click(function() {
            ...
            a.slice(-2);
            b <= 14 ? b += 2 : b -= 2,
            $("#post-content").css("font-size", b + "px")
        })
    },
    ...
    bindInsertMediumButton: function() {
        ...
        $("#imageInsertOk").bind("click", function() {
            ...
            b = '<img src="' + b + '">',
            handsome_enhance.insertContentInText(a, b, "#imageInsertModal")
        }),
        ...
        $("#musicInsertOk").bind("click", function() {
            ...
            $.ajax({
                ...
                success: function(c) {
                    b = c,
                    handsome_enhance.insertContentInText(a, b, "#musicInsertModal")
                }
            })
        })
    },
    insertContentInText: function(a, b, c) {
        ...
        document.selection) {
            ...
            var d = a.selectionStart
              , e = a.selectionEnd
              , f = d;
            a.value = a.value.substring(0, d) + b + a.value.substring(e, a.value.length),
            f += b.length,
            a.selectionStart = f,
            a.selectionEnd = f,
            a.focus()
  ...
  , handsome_content = {
    ...
    comment_ajax_complete: function(a) {
        ...
        handsome_content.highlight(a, !1)
    },
    pjax_complete: function() {
        this.featherReplace = !1,
        this.postSingleParse = !1,
        this.allSingleParseFlag = !1,
        null != handsome_content.toc_tree && (window.removeEventListener("scroll", handsome_content.tocScrollEvent),
        $("#tag_toc").length || (handsome_content.toc_tree.tocify("destroy"),
        handsome_content.toc_tree = null)),
        this.initAndPjax_Complete()
    },
    initAndPjax_Complete: function() {
        ...
        void 0 !== LocalConst.TIP_TITLE && ($.message({
            title: LocalConst.TIP_TITLE,
            message: LocalConst.TIP_CONTENT,
            type: "info",
            time: "5000"
        }),
        LocalConst.TIP_TITLE = void 0,
        LocalConst.TIP_CONTENT = void 0)
    },
    contentInit: function() {
        handsome_content.parseContainer("md_handsome_origin", "md_handsome", "handsome_md_text"),
        handsome_content.readModeInit(),
        handsome_content.parseContainer("comments", "", ""),
        handsome_content.allSingleParse()
    },
            ...
            try {
                $("[data-morphing]").fancyMorph({
                    ...
                    showComplete: function() {
                        "true" !== $("#morphing").attr("click_status") && ($("#morphing").attr("click_status", "true"),
                        handsome_content.parseContainer("morphing-content-real_origin", "morphing-content-real", "morphing-content-real_text", !0))
                    }
                }),
    ...
    parseContainer: function(a, b, c, d) {
        d = void 0 !== d && d,
        "" !== b && $("#" + b).length > 0 ? $("#" + c).length > 0 && "true" !== $("#" + c).attr("click_status") && ($("#" + c).attr("click_status", "true"),
        handsome_util.addScript(LocalConst.VDITOR_CDN + "/dist/method.min.js", "vditor_method", function() {
            handsome_content.vditorParse(b, c)
        })) : (d && "true" !== $("#" + a).attr("click_status") && ($("#" + a).html($("#" + a + "_text").text()),
        this.allSingleParseFlag = !1,
        feather.replace(),
        handsome_content.allSingleParse(a)),
        handsome_content.originParse(a))
    },
    ...
    highlightMacStyle: function(a) {
        var b = handsome_content.getSuiteHighlightStyleClassName();
        -1 !== b.indexOf("mac") && $(handsome_util.getChildName(a, "pre")).each(function(a, c) {
    ...
    highlightEnhance: function(a) {
        $(handsome_util.getChildName(a, "pre code")).each(function(a, b) {
            ...
            j.forEach(function(a) {
                var b = "";
                h && (f.textContent = a || "\n",
                b = ' style="height:' + f.getBoundingClientRect().height + 'px"'),
                i += "<span" + b + "></span>"
            }),
            f.style.display = "none",
            i = '<span class="vditor-linenumber__rows">' + i + "</span>",
    ...
    highlight: function(a, b) {
        if (LocalConst.THEME_HIGHLIGHT_CODE) {
            var c = handsome_content.getSuiteHighlightStyleFileName();
            -1 === LocalConst.HIGHLIGHT_CDN.indexOf("vditor") && (c += ".min"),
            b ? handsome_util.addOrReplaceStyle(LocalConst.HIGHLIGHT_CDN + "/styles/" + c + ".css", "vditorHljsStyle") : handsome_util.addStyle(LocalConst.HIGHLIGHT_CDN + "/styles/" + c + ".css", "vditorHljsStyle"),
    ...
    allSingleParse: function(a) {
        this.allSingleParseFlag || (void 0 === a && (a = "content"),
        this.allSingleParseFlag = !0,
        handsome_content.highlight(a, !1),
        LocalConst.POST_MATHJAX && "vditor" !== LocalConst.EDITOR_CHOICE && handsome_util.addScript(LocalConst.MATHJAX_SVG_CDN, "mathjax_js", function(b) {
            b && window.MathJax.typesetPromise([document.querySelector("#" + a)])
        }))
    },
    originParse: function(a) {
        $("#" + a).length > 0 && "true" !== $("#" + a).attr("click_status") && ($("#" + a).attr("click_status", "true"),
        handsome_content.md_parse_after(a))
    },
        ...
        Vditor.preview(document.getElementById(a), c, {
            ...
            after: function() {
                ...
                try {
                    ...
                    handsome_util.settingError("Handsome插件设置————vditor回调函数"),
                    console.error(a)
                }
            }
        })
    ...
    tab_enhance: function(a) {
        $(handsome_util.getChildName(a, 'a[data-toggle="tab"]')).on("shown.bs.tab", function(a) {
            ...
            g > h && (g = h),
            g >= 0 && smoothScroll(g, 200, null, b, !0)
        })
    },
    post_speech: function(a) {
        if ($(".speech-button").length > 0) {
            ...
            c ? (b.voice = c,
            b.onend = function() {
                $(".speech-button").removeClass("speech-current"),
                speechSynthesis.cancel(),
                $(".speech-button").html(feather.icons["play-circle"].toSvg({
                    width: "16px",
                    height: "16px"
                }))
            }
            ...
            $(".speech-button").on("click", function() {
                const a = $(this);
                $(this).hasClass("speech-current") ? $(this).hasClass("paused") ? (speechSynthesis.resume(),
                feather.icons.circle.toSvg(),
                $(".speech-button").html(feather.icons["pause-circle"].toSvg({
                    width: "16px",
                    height: "16px"
                })),
                $(this).removeClass("paused")) : (speechSynthesis.pause(),
                $(".speech-button").html(feather.icons["play-circle"].toSvg({
                    width: "16px",
                    height: "16px"
                })),
                $(this).addClass("paused")) : (speechSynthesis.speak(b),
                $(this).addClass("speech-current"),
                $(".speech-button").html(feather.icons["pause-circle"].toSvg({
                    width: "16px",
                    height: "16px"
                }))),
    ...
    getVoice: function() {
        ...
        return b.forEach(function(b) {
            b.lang === a.replace("_", "-") && b.localService && (c = b),
            b.default && (d = b)
        }),
        c || (c = d),
        c
    ...
    urlOpen: function(a) {
        a = -1 !== a.indexOf("comment") ? handsome_util.getChildName(a, ".comment-content-true") : "#" + a,
        $(a + " a:not(.light-link):not(.no-external-link)").each(function() {
            const a = $(this).attr("href");
            a && (-1 !== a.indexOf(LocalConst.BLOG_URL_N) || -1 === a.indexOf("http") || ($(this).attr("target", "_blank"),
            LocalConst.NO_LINK_ICO || ($(this).prepend('<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-external-link"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg>'),
            $(this).prop("outerHTML", '<span class="external-link">' + $(this).prop("outerHTML") + "</span>"))))
        })
    },
    md_parse_after: function(a) {
        handsome_content.table_wrap(a),
        handsome_content.setWechatAndio(a),
        handsome_content.setVideoTag(a),
        handsome_content.switchVideoEvent(a),
        !this.featherReplace && $("#md_handsome").length > 0 && -1 !== a.indexOf("md_handsome") && (feather.replace(),
        this.featherReplace = !0),
        handsome_content.urlOpen(a),
        handsome_content.setFancyBox(a),
        handsome_content.tab_enhance(a),
        this.postSingleParse || -1 === a.indexOf("md_handsome") || (this.postSingleParse = !0,
        window.speechSynthesis ? handsome_content.post_speech(a) : $(".speech-button").hide(),
        $("#support_author").on("click", function() {
    ...
    switchVideoEvent: function(a) {
        $("#" + a + " .video-player>.panel-collapse").on("click", function() {
            $(this).attr("data-src") && "true" !== $(this).attr("click-status") && ($("#" + $(this).attr("data-iframe")).attr("src", $(this).attr("data-src")),
            $(this).attr("click-status", "true"))
        }),
        $("#" + a + " .video-player .btn-group label").on("click", function() {
            const b = $(this).attr("data-src");
            $("#" + a + " #title_" + $(this).attr("data-iframe")).text($(this).text().trim()),
            $("#" + a + " #" + $(this).attr("data-iframe")).attr("src") !== b && ($("#" + a + " #" + $(this).attr("data-iframe")).attr("src", $(this).attr("data-src")),
            $("#" + a + " #origin_" + $(this).attr("data-iframe")).attr("href", $(this).attr("data-origin")))
        })
    },
    setVideoTag: function(a) {
        $("#" + a + " video").each(function() {
            $(this).hasClass("dplayer-video") || (void 0 !== $(this).attr("controls") && $(this).removeAttr("controls"),
            $(this).wrap('<div class="kyt-player embed-responsive embed-responsive-16by9"></div>'),
            $(this).parent().append('<div class="play-button"></div>'),
            $(this).on("loadedmetadata", function() {
                $(this).attr("loaded", !0)
            }),
            $(this).on("playing", function() {
                $(this).parent(".kyt-player").hasClass("embed-responsive embed-responsive-16by9") && ($(this).parent(".kyt-player").removeClass("embed-responsive embed-responsive-16by9"),
                $(this).parent(".kyt-player").css("width", Math.min($(this).parent(".kyt-player").width(), $(this)[0].videoWidth)),
                $(this).parent(".kyt-player").css("height", $(this).parent(".kyt-player").width() * $(this)[0].videoHeight / $(this)[0].videoWidth))
            }),
            $(this).bind("click", function() {
                $(this).next(".play-button").is(":hidden") && ($(this).next(".play-button").css("display", "block"),
        ...
        $("#" + a).delegate(".play-button", "click", function(a) {
            $(this).prev("video").css("background-image", "none"),
            $(this).is(":visible") && ($(this).css("display", "none"),
    ...
    toc: function(a) {
        ...
        $("#" + b).length > 0 ? (handsome_content.toc_tree && (handsome_content.toc_tree.tocify("destroy"),
        handsome_content.toc_tree = null),
        handsome_content.toc_tree = $("#" + b).tocify({
            context: "#" + a,
        ...
        window.innerWidth > 992 && !LocalConst.NO_SHOW_RIGHT_SIDE_IN_POST ? (startPos = 0,
        $("#tag_toc").length > 0 && (startPos = Number($("#tag_toc").offset().top) - Number(parseInt(LocalConst.OFF_SCROLL_HEIGHT)),
        $("#tag_toc").css("top", Number(parseInt(LocalConst.OFF_SCROLL_HEIGHT)))),
        $(".tocify-mobile-panel").hide()) : $("#tag_toc").hide(),
        null != handsome_content.toc_tree && window.addEventListener("scroll", handsome_content.tocScrollEvent)) : handsome_content.toc_tree = null
    },
        ...
        $(handsome_util.getChildName(a, ".album_block")).shortcode({
            album: function() {
                ...
                for (f += c ? "<div class='album-photos'>" : "<div class='photos'>"; null !== (d = a.exec(this.contents)); ) {
                    ...
                    var g = b.exec(d[5])
                      , h = null == g ? "" : g[1];
                    c ? (f += "<figure>" + d[0].toString() + "<figcaption>",
                    f += h.toString() + "</figcaption></figure>") : f += '<figure class="image-thumb" itemprop="associatedMedia" itemscope="" itemtype="http://schema.org/ImageObject">' + d[0] + '<figcaption itemprop="caption description">' + h + "</figcaption></figure>"
                }
                return e || (f = '<small class="text-muted letterspacing indexWords">相册无图片</small>'),
                f += "</div>"
            }
        })
    },
    setPhotosAuto: function(a) {
        $.each($(handsome_util.getChildName(a, ".album-photos figure")), function(a, b) {
            ...
            if ("1" === LocalConst.LAZY_LOAD) {
                ...
                c.src = $(b).find("img").attr("src");
            c.onload = function() {
                var a = parseFloat(c.width)
                  , d = parseFloat(c.height);
                $(b).css("flex-grow", 50 * a / d),
                $(b).find("a").css("padding-top", d / a * 100 + "%")
            }
        })
    },
    setImageFancyBox: function(a) {
        ...
        $(handsome_util.getChildName(a, "img")).each(function() {
            var a = $(this);
            if (void 0 !== a.attr("alt"))
                var b = a.attr("alt");
            else
                var b = "";
            var c = a.parent("a")
            ...
            if (void 0 !== a.attr("max") && a.wrap('<div class="max-img overflow-y-auto small-scroll-bar"></div>'),
            "undefined" === d && (!c || c.size() < 1)) {
                var g;
                g = this.getAttribute("data-src") || this.getAttribute("data-original") || this.getAttribute("src"),
                g = g.replace(/(.*?)[!|\?]\/.*/, "$1");
                var h = a.attr("class")
                  , i = "";
                void 0 !== h && -1 !== h.indexOf("emotion") || (a.parent().hasClass("image-thumb") || a.parent().parent().hasClass("album-photos") || !LocalConst.SHOW_IMAGE_ALT || "undefined" !== e || (i = '<figcaption class="post-img-figcaption">' + b + "</figcaption>"),
                a.addClass("block"));
                var j = a.prop("outerHTML");
                j += i,
                "undefined" !== f ? a.prop("outerHTML", '<a class="light-link img-blur" data-fancybox="gallery" style="background-image: url(' + g + ')" no-pjax data-type="image" data-caption="' + b + '" href="' + g + '">' + j + "</a>") : a.prop("outerHTML", '<a class="light-link" data-fancybox="gallery" no-pjax data-type="image" data-caption="' + b + '" href="' + g + '">' + j + "</a>")
            }
        })
    },
    setFancyBox: function(a) {
        handsome_content.parsePhotos(a),
        handsome_content.setImageFancyBox(a),
        handsome_content.setPhotosAuto(a),
        "1" === LocalConst.LAZY_LOAD && $(handsome_util.getChildName(a, "img")).lazyload({
            effect: "fadeIn",
            threshold: "200",
            vertical_only: !0
        })
    ...
    reInitDPlayer: function(container) {
        if (!("undefined" == typeof dPlayerOptions || "undefined" == typeof dPlayers || $(".dplayer").length <= 0)) {
            ...
            for (var len = dPlayerOptions.length, i = 0; i < len; i++)
    ...
    tocScrollEvent: function() {
        ...
        window.innerWidth > 992 && !LocalConst.NO_SHOW_RIGHT_SIDE_IN_POST && (("undefined" == typeof startPos || startPos < 0) && (startPos = Number($("#tag_toc").offset().top) - Number(parseInt(LocalConst.OFF_SCROLL_HEIGHT))),
        a > startPos ? $("#tag_toc").addClass("fixed") : $("#tag_toc").removeClass("fixed"))
    }
}
  , handsome_music = {
    post_APlayer_list: [],
    ...
    init: function() {
        handsome_music.initIndexMusic(),
        handsome_music.initAndPjax_Complete()
    },
    pjax_click: function() {},
    pjax_complete: function() {
        this.destroyMusic(),
        ...
        $(handsome_util.getChildName(a, ".handsome_aplayer")).each(function() {
            ...
            $(this).children(".handsome_aplayer_music").each(function(a) {
                ...
                if (h.dataset.id) {
                    ...
                    i = i.replace(":server", h.dataset.server),
                    i = i.replace(":type", h.dataset.type),
                    i = i.replace(":id", h.dataset.id),
                    i = i.replace(":auth", h.dataset.auth),
                    i = i.replace(":r", Math.random()),
                    $.ajax({
                        type: "GET",
                        url: i,
                        error: function(a) {
                    ...
                    d.push(j),
                    f = handsome_music.checkPlayerDataLoaded(g, f, e, d, j.lrc, b, c)
                }
            })
        })
    },
    checkPlayerDataLoaded: function(a, b, c, d, e, f, g) {
        return b++,
        b === c && handsome_music.buildAPlayer(a, d, e, f, g),
        b
    },
    buildAPlayer: function(a, b, c, d, e) {
        ...
        if (b.length) {
            ...
            for (var h in f) {
                var i = h.toLowerCase();
                (a.dataset.hasOwnProperty(i) || a.dataset.hasOwnProperty(h) || null !== f[h]) && (g[h] = a.dataset[i] || a.dataset[h] || f[h],
                "true" !== g[h] && "false" !== g[h] || (g[h] = "true" == g[h]))
            }
            if (d) {
                b.length <= 1 && $("#global_player_toggle").hide(),
                handsome_music.global_player = new APlayer(g);
                try {
                    if (LocalConst.SHOW_LYRIC || handsome_music.global_player.lrc.hide(),
                    LocalConst.AUTO_SHOW_LYRIC && (handsome_music.global_player.on("play", function() {
                ...
                "comment" === e ? this.comment_APlayer_list.push(new APlayer(g)) : this.post_APlayer_list.push(new APlayer(g));
    ...
    destroyMusic: function() {
        ...
        handsome_music.destroyPostMusic()
    },
  ...
  , handsome_pjax = {
    ...
    initCommentPjax: function() {
        ...
        $("#content").pjax && $("#content").pjax('#comment-navigation a[href^="' + LocalConst.BLOG_URL_N + '"]:not(a[target="_blank"]):not([no-pjax])', {
            ...
            handsome_pjax.comment_pjax_clicked && (handsome_UI.commentSkeleton(),
            handsome_core.onlyPjax.comment_pjax_click())
        }).on("pjax:complete", function() {
            if (handsome_pjax.comment_pjax_clicked) {
                handsome_core.onlyPjax.comment_pjax_complete(),
                $("#post-comment-list .loading-nav").addClass("hide");
    ...
    init: function() {
        this.initAndPjax_Complete(),
        $(document).pjax && $(document).pjax('a[href^="' + LocalConst.BLOG_URL_N + '"]:not(a[target="_blank"]):not([no-pjax]), a[data-pjax]', {
            container: "#content",
            fragment: "#content",
            timeout: 8e3
        }).on("pjax:send", function() {}).on("pjax:click", function() {
            ...
            handsome_pjax.comment_pjax_clicked || (handsome_core.onlyPjax.pjax_complete(),
            "1" === LocalConst.LAZY_LOAD && ("1" === LocalConst.PAGE_ANIMATE ? $(".app-content-body").animateCss("fadeIn", function() {
                $(".lazy").lazyload({
                    effect: "fadeIn",
                    threshold: "500"
                })
    ...
    pjax_click: function() {
        if (handsome_UI.showAnimate(),
        "auto" === LocalConst.PJAX_TO_TOP) {
            if ("default" === LocalConst.PJAX_ANIMATE) {
                ...
                if (a) {
                    ...
                    $("#loading").removeClass("loading-sticky"),
                    $("body,html").animate({
                        scrollTop: 0
                    }, Number(LocalConst.TO_TOP_SPEED))
            }
        } else
            "0" === LocalConst.PJAX_TO_TOP && $("body,html").animate({
                scrollTop: 0
            }, Number(LocalConst.TO_TOP_SPEED))
    },
    pjax_complete: function() {
        ...
        try {
            ...
            handsome_util.settingError("外观设置——pjax设置——回调函数"),
            console.error(a)
        }
        this.initAndPjax_Complete()
    },
  ...
  , handsome_resize = {
    ...
    pjax_click: function() {
        this.cancelLeftBarListener(),
        this.cancelRightBarListener()
    },
    pjax_complete: function() {
        this.checkResize()
    },
    checkResize: function() {
        handsome_resize._needRecover = !1,
        handsome_resize._startX = 0,
        this.correctLeftSwitch(),
        this.correctRightSwitch(),
        this.correctPosition()
    },
    initResize: function() {
        this.checkResize(),
        handsome_resize.left_max_width = $("#aside").width() + 20,
        handsome_resize.right_max_width = $("#rightAside").width() + 40
    },
    needResize: function() {
        this.checkResize()
    },
    reSet: function() {
        ...
        $("#content").removeClass("right-aside-folded"),
        this.checkResize()
    },
    removeDragListener: function(a) {
        document.removeEventListener && (document.removeEventListener("mousemove", this.handle_list[a].mousemove, !0),
        document.removeEventListener("mouseup", this.handle_list[a].mouseup, !0))
    },
    addDragListener: function(a) {
        document.addEventListener && (document.addEventListener("mousemove", this.handle_list[a].mousemove, !0),
        document.addEventListener("mouseup", this.handle_list[a].mouseup, !0))
    },
    correctPosition: function() {
        $("#content").hasClass("left-aside-folded") ? ($("#trigger_left_button").css("left", $("#aside").offset().left + "px"),
        this.triggerLeftPanel(!1)) : (this.triggerLeftPanel(!0),
        $("#trigger_left_content").css("left", $("#aside").width())),
        $("#content").hasClass("right-aside-folded") ? ($("#trigger_right_button").css("left", $("#trigger_right_content").offset().left - 22 + "px"),
        this.triggerRightPanel(!1)) : (this.triggerRightPanel(!0),
        handsome_enhance.rightAsideWidth > 0 && $("#rightAside").css("width", handsome_enhance.rightAsideWidth + "px"))
    },
    correctLeftSwitch: function() {
        return LocalConst.CLOSE_LEFT_RESIZE || window.innerWidth < 760 || LocalConst.IS_TRANSPARENT || $("#alllayout").hasClass("app-aside-dock") || !$("#alllayout").hasClass("app-aside-fix") || $("#alllayout").hasClass("app-aside-folded") ? (handsome_resize.closeLeftResize(),
        !1) : (handsome_resize.openLeftResize(),
        !0)
    },
    correctRightSwitch: function() {
        return LocalConst.CLOSE_RIGHT_RESIZE || window.innerWidth < 984 ? (handsome_resize.closeRightResize(),
        !1) : (handsome_resize.openRightResize(),
        !0)
    },
    closeRightResize: function() {
        $("#content").removeClass("right-aside-folded"),
        this.triggerRightPanel(!0),
        $("#trigger_right_content").hide(),
        this.cancelRightBarListener()
    },
    closeLeftResize: function() {
        $("#trigger_left_content").hide(),
        $("#content").removeClass("left-aside-folded"),
        this.triggerLeftPanel(!0),
        this.cancelLeftBarListener()
    },
    openRightResize: function() {
        $("#trigger_right_content").show(),
        this.registerRightBarListener()
    },
    openLeftResize: function() {
        $("#trigger_left_content").show(),
        this.registerLeftBarListener()
    },
    setLeftSize: function(a) {
        $("#content").css("margin-left", a),
        $("#footer").css("margin-left", a),
        "" === a ? $("#trigger_left_content").css("left", $("#aside").width()) : $("#trigger_left_content").css("left", a),
        $("#header_right").css("margin-left", a),
        $("#header_left").css("width", a)
    },
    triggerLeftPanel: function(a) {
        if (a)
            ...
            $("#aside-user").css("visibility", "visible");
        else {
            $("#aside").css("visibility", "hidden"),
            $("#aside-user").css("visibility", "hidden");
            const b = $("#aside").offset().left;
            this.setLeftSize("0px"),
            $("#content").addClass("left-aside-folded"),
            $("#trigger_left_button").css("left", b)
        }
    },
    triggerRightPanel: function(a) {
        a ? ($("#content").removeClass("right-aside-folded"),
        handsome_enhance.rightAsideWidth = -1) : ($("#content").addClass("right-aside-folded"),
        $("#rightAside").css("width", ""),
        $("#trigger_right_button").css("left", $("#trigger_right_content").offset().left - 22 + "px"),
        handsome_enhance.rightAsideWidth = -1)
    },
    cancelLeftBarListener: function() {
        $("#trigger_left_button").off("click"),
        $("#trigger_left_content").off("mousedown"),
        this.handle_list.left.isRegister = !1
    },
    cancelRightBarListener: function() {
        $("#trigger_right_button").off("click"),
        $("#trigger_right_content").off("mousedown"),
        this.handle_list.right.isRegister = !1
    },
    registerLeftBarListener: function() {
        if (!handsome_resize.handle_list.left.isRegister) {
            handsome_resize.handle_list.left.isRegister = !0,
            $("#trigger_left_button").on("click", function() {
                handsome_resize.triggerLeftPanel(!0)
            });
            const a = function(a) {
                handsome_resize.is_left_drag = !0,
                a || (a = window.event),
                $("#aside").css("visibility", "visible"),
                $("#aside-user").css("visibility", "visible");
                const b = parseInt(a.clientX)
                  , c = $("#aside").offset().left;
                b - c >= 0 && b - c <= handsome_resize.left_max_width ? ($("#trigger_left_content").hasClass("trigger_drag") || $("#trigger_left_content").addClass("trigger_drag"),
                handsome_resize.setLeftSize(b - c + "px"),
                b - c > 200 && $("#aside").css("width", b - c + "px")) : $("#trigger_left_content").removeClass("trigger_drag"),
                a.stopPropagation ? a.stopPropagation() : a.cancelBubble = !0
            }
              , b = function(a) {
                handsome_resize.is_left_drag = !1,
                $("body").removeClass("no-select"),
                $("#content").removeClass("resize-drag"),
                a || (a = window.event),
                handsome_resize.removeDragListener("left"),
                $("#trigger_left_content").removeClass("trigger_drag");
                const b = $("#aside").offset().left
                  , c = parseInt(a.clientX);
                c - b < $("#aside").width() ? c - b < $("#aside").width() / 2 ? handsome_resize.triggerLeftPanel(!1) : handsome_resize.triggerLeftPanel(!0) : $("#content").removeClass("left-aside-folded"),
                a.stopPropagation ? a.stopPropagation() : a.cancelBubble = !0
            };
            this.handle_list.left.mousemove = a,
            this.handle_list.left.mouseup = b,
            $("#trigger_left_content").on("mousedown", function(a) {
                $("body").addClass("no-select"),
                $("#post-panel").height() < $("#aside").height() - 100 && $("#post-panel").css("height", $("#aside").height() - 100),
                $("#content").addClass("resize-drag"),
                handsome_resize.addDragListener("left")
            }),
    ...
    registerRightBarListener: function() {
        if (!handsome_resize.handle_list.right.isRegister) {
            handsome_resize.handle_list.right.isRegister = !0,
            $("#trigger_right_button").on("click", function() {
                handsome_resize.triggerRightPanel(!0),
                handsome_resize._needRecover = !1,
                handsome_resize._startX = 0,
                $("#toc").css("width", $("#rightAside").width() - 40 + "px")
            });
            const a = function(a) {
                handsome_resize.is_right_drag = !0,
                a || (a = window.event);
                const b = parseInt(a.clientX);
                0 === handsome_resize._startX && (handsome_resize._startX = b);
                const c = b - handsome_resize._startX;
                if (handsome_resize._needRecover)
                    handsome_resize.triggerRightPanel(!0),
                    handsome_resize._startX = 0;
                else {
                    const d = $("#rightAside").width() - c;
                    d > handsome_resize.right_min_width && d < handsome_resize.right_max_width ? (handsome_enhance.rightAsideWidth = d,
                    $("#rightAside").css("width", d + "px"),
                    $("#toc").css("width", d - 40 + "px"),
                    handsome_resize._startX = b,
                    $("#trigger_right_content").hasClass("trigger_drag") || $("#trigger_right_content").addClass("trigger_drag")) : (d <= handsome_resize.right_min_width && handsome_resize.triggerRightPanel(!1),
                    $("#trigger_right_content").removeClass("trigger_drag"))
                }
                a.stopPropagation ? a.stopPropagation() : a.cancelBubble = !0
            }
              , b = function(a) {
                handsome_resize.is_right_drag = !1,
                $("#trigger_right_content").removeClass("trigger_drag"),
                handsome_resize._needRecover = $("#content").hasClass("right-aside-folded"),
                $("body").removeClass("no-select"),
                a || (a = window.event),
                handsome_resize.removeDragListener("right"),
                a.stopPropagation ? a.stopPropagation() : a.cancelBubble = !0
            };
            this.handle_list.right.mousemove = a,
            this.handle_list.right.mouseup = b,
            $("#trigger_right_content").on("mousedown", function(a) {
                $("body").addClass("no-select"),
                handsome_resize.addDragListener("right")
            }),
            $("#trigger_right_content").on("mouseover", function(a) {
                $("body").addClass("no-select"),
                $("#trigger_right_content").height($("#alllayout").height() - $("#header").height())
            }),
  ...
  , handsome_search = {
    ...
    up_down_select_enter: function(a, b) {
        ...
        "enter" === b.key)
            return -1 === f && handsome_search.drop_menu_is_showing ? $("#searchform1").submit() : $(e[f]).children("a").click(),
            !1;
        var g = (e.length + f + d) % e.length;
        $(e[f]).removeClass("active"),
        $(e[g]).addClass("active"),
        handsome_search.scrollTo($(e[g]), $(c), 5)
    },
    ...
    set_search_data: function() {
        function a() {
            handsome_search.modal_is_showing && !handsome_search.search_data_status[b] && handsome_search.search_data[b] ? ($("#search_modal_input").attr("disabled", !1),
            handsome_search.set_loading_status("search_modal_submit", !1),
            $("#search_modal_input").val() && handsome_search.search_action($("#search_modal_input").val()),
            $("#search_modal_input").select()) : ($("#search_modal_input").attr("disabled", !0),
            handsome_search.set_loading_status("search_modal_submit", !0))
        }
        const b = handsome_search.search_type;
        if (handsome_search.search_data[b] || handsome_search.search_data_status[b])
            a();
        else {
            handsome_search.search_data_status[b] = !0,
            $("#search_modal_input").attr("disabled", !0),
            handsome_search.set_loading_status("search_modal_submit", !0);
            ...
            $.getJSON(LocalConst.BLOG_URL + "?action=get_search_cache&type=" + b + "&v=" + c + "&l=" + LocalConst.USER_LOGIN, function(c) {
                handsome_search.search_data_status[b] = !1,
                handsome_search.search_data[b] = c,
                a()
            })
        }
    },
    searchPopup: function() {
        ...
        hotkeys(a, function() {
            return $("#search_modal").modal("toggle"),
            !1
        }),
        $("#search_modal").on("shown.bs.modal", function(a) {
            handsome_search.modal_is_showing = !0,
            -1 !== window.location.href.indexOf("cross") ? ($("#search_modal .nav-link:eq(1)").tab("show"),
            handsome_search.search_type = "TIME_COMMENT") : ($("#search_modal .nav-link:first").tab("show"),
            handsome_search.search_type = "POST"),
            $('#search_modal a[data-toggle="tab"]').on("shown.bs.tab", function(a) {
                "#post_search_content" === a.target.dataset.target ? handsome_search.search_type = "POST" : handsome_search.search_type = "TIME_COMMENT",
                handsome_search.set_search_data()
            }),
            handsome_search.trigger_up_down_select_hotkeys(!0),
            $("#search_modal").on("hidden.bs.modal", function(a) {
                handsome_search.modal_is_showing = !1,
                handsome_search.trigger_up_down_select_hotkeys(!1)
            }),
            handsome_search.set_search_data();
            var b = null;
    ...
    search_action: function(a) {
        ...
        a.trim()) {
            ...
            b.empty(),
            b.append(handsome_search.parse_search_results_to_html(c, a, handsome_search.search_type))
        } else
            for (var d in handsome_search.search_container)
                $(handsome_search.search_container[d]).empty()
    },
    searchTips: function() {
        handsome_search.searchInput(),
    ...
    trigger_up_down_select_hotkeys: function(a) {
        a ? handsome_search.up_down_select_hotkey_is_bind || (handsome_search.up_down_select_hotkey_is_bind = !0,
        hotkeys("up,down,enter", handsome_search.up_down_select_enter)) : handsome_search.modal_is_showing || handsome_search.drop_menu_is_showing || !handsome_search.up_down_select_hotkey_is_bind || (hotkeys.unbind("up,down,enter", handsome_search.up_down_select_enter),
        handsome_search.up_down_select_hotkey_is_bind = !1)
    },
    trigger_search_drop: function(a) {
        handsome_search.drop_menu_is_showing = a,
        a ? (handsome_search.trigger_up_down_select_hotkeys(!0),
        $("#search_tips_drop").text(""),
        $("#search_tips_drop").removeClass("hide")) : (handsome_search.trigger_up_down_select_hotkeys(!1),
        $("#search_tips_drop").text(""),
    ...
    searchInput: function() {
        ...
        $("#search_input").on("input", function() {
            setTimeout(function() {
                a || (null !== c && clearTimeout(c),
                c = setTimeout(function() {
                    var a = $("#search_input").val();
                    if ("" !== a.trim()) {
                        handsome_search.set_loading_status("search_submit", !0),
                        null !== b && (b.abort(),
                        b = null),
                        b = $.getJSON(LocalConst.BLOG_URL + "?action=ajax_search&content=" + a + "&v=" + LocalConst.LATEST_POST_TIME + "&l=" + LocalConst.USER_LOGIN, function(a) {
                            handsome_search.trigger_search_drop(!0),
                            $(a.results).appendTo("#search_tips_drop"),
                            handsome_search.set_loading_status("search_submit", !1),
                            b = null
                        }).fail(function() {})
                    } else
                        handsome_search.trigger_search_drop(!1);
    ...
    parse_search_results_to_html: function(a, b, c, d) {
        ...
        for (var j in a) {
            ...
            var l = "";
            for (var m in f)
                l += "(" + handsome_util.escapeRegex(f[m]) + ")|";
    ...
    set_loading_status: function(a, b) {
        b ? ($(handsome_util.getChildName(a, ".spin-search")).addClass("show inline"),
        $(handsome_util.getChildName(a, ".icon-search")).addClass("hide")) : ($(handsome_util.getChildName(a, ".spin-search")).removeClass("show inline"),
        $(handsome_util.getChildName(a, ".icon-search")).removeClass("hide"))
    },
    ...
    filter_data: function(a, b, c) {
        return a.input && c === a.input || (a.key_count = 0,
        a.key_type = 0),
        handsome_search.parse_words(c).filter(function(c) {
            const d = b.filter(function(b) {
                if (!a.hasOwnProperty(b))
                    return !1;
                const d = a[b].toUpperCase().indexOf(c.toUpperCase());
                return d > -1 ? (a.key_count += a[b].split(c).length - 1,
                "content" === b && (a.firstOccur = d),
                !0) : void 0
            });
            return d.length > 0 && (a.key_type += 1),
            d.length > 0
        }).length > 0
    },
  ...
  , handsome_setting = {
    init: function() {
        if (2 === Math.floor(3 * Math.random())) {
            ...
            b.u = LocalConst[a(c)],
            "1" !== b[m.call(null, 117)] && -1 === window.location.href.indexOf(window.atob("L2FkbWluL3ByZXZpZXcucGhw")) && (handsome_UI.c[0][a(handsome_basics.j)] = "")
        }
        if (LocalConst.SHOW_SETTING_BUTTON) {
            handsome_util.getPrefersDarkModeState(!1),
            $("body").on("click", ".mode_set input", function() {
                "dark" === handsome_UI.mode ? handsome_UI.mode = "light" : handsome_UI.mode = "dark",
                handsome_content.highlight("content", !0),
                $("#statistic_pane").attr("data-status", "false")
            });
            var d = {
                defaults: {
            ...
            function(a, b, c, d, e) {
                ...
                function f() {
                    function c() {
                        if (!s) {
                            ...
                            r = f().appendTo("body"),
                            j(),
                            handsome_setting.init_complete()
                        }
                    }
                    function f() {
                        var b = {
                          ...
                          , e = c(b)
                          , e = a(e);
                        return e.find(":input").change(h),
                        e.find("[name=reset]").click(i),
                        e
                    }
                    function g() {
                        p("header-fixed") && (LocalConst.OFF_SCROLL_HEIGHT = 55,
                        p("aside-dock") && p("aside-fix") && (LocalConst.OFF_SCROLL_HEIGHT = 115))
                    }
                    function h() {
                        var c = a(this).attr("name")
                          , d = n(c);
                        Number(b.innerWidth) < 767 && ("aside-fix" == c || "aside-folded" == c || "aside-dock" == c) || (k(c, d),
                        q(c, d)),
                        handsome_resize.initResize(),
                        "dark_mode" === c && (a("#mode_set span").html(LocalConst.DARK_MODE_FIXED),
                        a("#mode_set small").hide()),
                        p("aside-fix") && p("aside-dock") && !p("header-fixed") && (n("header-fixed", !0),
                        k("header-fixed", !0),
                        q("header-fixed", !0),
                        a.message({
                            title: "外观设置提示",
                        ...
                        p("aside-dock") && p("aside-folded") && (n("aside-folded", !1),
                        k("aside-folded", !1),
                        q("aside-folded", !1),
                        a.message({
                            title: "外观设置提示",
                    ...
                    function i() {
                        ...
                        a("#mode_set span").html(LocalConst.DARK_MODE_AUTO),
                        a("#mode_set small").show(),
                        q(null, null)
                    }
                    function j() {
                        a.each(p(), k),
                    ...
                    function k(b, c) {
                        ...
                        case "layout-boxed":
                            var d = (c ? "add" : "remove") + "Class"
                              ...
                              , f = a(".app");
                            "layout-boxed" === b ? (f[d]("container"),
                            a("html").addClass("bg"),
                            f[e]("no-container")) : f[d]("app-" + b),
                            "aside-dock" === b && f[d]("small-dock");
                            break;
                        case "color":
                            l(c);
                            break;
                        case "dark_mode":
                            "auto" !== c ? ("dark" !== c && 1 != c || handsome_UI.dark_mode(),
                            "light" !== c && 0 != c || handsome_UI.light_mode()) : "dark" === handsome_UI.mode ? handsome_UI.dark_mode() : "light" === handsome_UI.mode && handsome_UI.light_mode(),
                            m("#header > .navbar-header", "navbarHeader", n("color"))
                        }
                    }
                    function l(a) {
                        m("#header > .navbar-header", "navbarHeader", a),
                        m("#header > .collapse", "navbarCollapse", a),
                        m("#aside", "aside", a)
                    }
                    function m(b, c, e) {
                        if (void 0 !== e) {
                            for (var f = d.sections.colors, g = f[e], h = a(b), i = 0; i < f.length; ++i)
                                h.removeClass(f[i][c]);
                            h.addClass(g[c]),
                            "navbarHeader" == c && "light" === handsome_UI.mode && ("bg-white" == g[c] || "bg-white-only" == g[c] || "bg-light" == g[c] ? a("#navbar-brand-dark").length > 0 && (a("#navbar-brand-dark").addClass("hide"),
                            a("#navbar-brand-day").show()) : a("#navbar-brand-dark").length > 0 && (a("#navbar-brand-dark").removeClass("hide"),
                            a("#navbar-brand-day").hide()))
                        }
                    }
                    function n(b, c) {
                        var d;
                        return r.find(":input").each(function() {
                            ...
                            if (g.attr("name") === b) {
                                ...
                                c === e)
                                    ...
                                    switch (f) {
                                    case "checkbox":
                                        "dark_mode" === b && "auto" === c ? (handsome_util.getPrefersDarkModeState(!0),
                                        g.prop("checked", "dark" === handsome_UI.mode)) : g.prop("checked", 1 == c || "dark" === c);
                                        break;
                                    case "radio":
                                        if (g.val() != c)
                                            return;
                    ...
                    function q(b, c) {
                        var f, g = d.localStorageKey, h = t.getItem(g);
                        return f = h ? JSON.parse(h) : {},
                        b ? c === e ? f[b] : (null === c ? delete f[b] : f[b] = c,
                        a.isEmptyObject(f) ? t.removeItem(g) : t.setItem(g, JSON.stringify(f))) : null === b && null === c ? t.removeItem(g) : f
                    }
                    var r, s = !1, t = b.localStorage;
                    return t && a(c),
                    {
                        load: c,
                ...
                b.handsome = b.handsome || {},
                b.handsome.customizer = new f
            }(jQuery, window, document, d)
        }
    },
    init_complete: function() {
        $(".setting_body_panel i").tooltip(),
        feather.replace(),
        $(".setting_body_panel button").tooltip(),
        handsome_resize.initResize(),
        handsome_UI.leftBar()
    },
    pjax_complete: function() {
...
!function(a) {
    ...
    a.fn.extend({
        tocify: function(b) {
            if ("destroy" === b)
                a(window).off("scroll.tocify"),
                a(d.options.tocContainer).addClass("hide"),
                this.html("");
            else if (d.options = a.extend({}, d.options, b),
            a(d.options.context).length > 0) {
                if (!d._create(b, this))
                    return a(d.options.tocContainer).addClass("hide"),
                    a(window).off("scroll.tocify"),
                    null;
    ...
    const d = {
        ...
        _create: function(b, c) {
            ...
            return d.element = c,
            d.clicked = !1,
            d.lastActiveElement = c,
            d.start = d.element.get(0).scrollTop || window.pageYOffset,
            d.extendPageScroll = !0,
            d.items = [],
            d._generateToc(),
            !d.noToc && (d._addCSSClasses(),
            d.webkit = function() {
                for (var a in window)
                    if (a && -1 !== a.toLowerCase().indexOf("webkit"))
                        return !0;
                return !1
            }(),
            d._setEventHandlers(),
            a(window).load(function() {
                d._setActiveElement(!0),
        ...
        _generateToc: function() {
            var b, c, d = this, e = d.options.ignoreSelector;
            if (null == (b = -1 !== this.options.selectors.indexOf(",") ? this._getFirstItem(this.options.selectors) : a(this.options.context).find(this.options.selectors.replace(/ /g, ""))) || !b.length)
                return d.element.addClass("tocify-hide"),
                d.noToc = !0,
                void a(d.options.tocContainer).addClass("hide");
            d.noToc = !1,
            d.element.addClass("tocify"),
            b.each(function(b) {
                a(this).is(e) || (c = a("<ul/>", {
                    id: "tocify-header" + b,
                    class: "tocify-header"
                }).append(d._nestElements(a(this), b)),
                d.element.append(c),
                a(this).nextUntil(this.nodeName.toLowerCase()).each(function() {
        ...
        _setActiveElement: function(a) {
            ...
            return c.length ? (b.element.find("." + b.focusClass).removeClass(b.focusClass),
            d.addClass(b.focusClass),
            b.options.showAndHide && d.click()) : (b.element.find("." + b.focusClass).removeClass(b.focusClass),
            !c.length && a && b.options.highlightDefault && b.element.find(".tocify-item").first().addClass(b.focusClass)),
            b
        },
        _nestElements: function(b, c) {
            ...
            d.length ? this.items.push(b.text() + c) : this.items.push(b.text()),
            f = this._generateHashValue(d, b, c),
            e = a("<li/>", {
                class: "tocify-item",
                "data-unique": f
            }).append(a("<a/>", {
        ...
        _generateHashValue: function(a, b, c) {
            ...
            if ("pretty" === e) {
                for (d = b.text().toLowerCase().replace(/\s/g, "-"); d.indexOf("--") > -1; )
                    d = d.replace(/--/g, "-");
                for (; d.indexOf(":-") > -1; )
                    d = d.replace(/:-/g, "-")
            } else
                d = "function" == typeof e ? e(b.text(), b) : b.text().replace(/\s/g, "");
            return a.length && (d += "" + c),
            d
        },
        ...
        _setEventHandlers: function() {
            ...
            this.element.on("click.tocify", "li", function(c) {
                if (b.options.history && (window.location.hash = a(this).attr("data-unique")),
                b.element.find("." + b.focusClass).removeClass(b.focusClass),
                a(this).addClass(b.focusClass),
                b.options.showAndHide) {
            ...
            (b.options.extendPage || b.options.highlightOnScroll || b.options.scrollHistory || b.options.showAndHideOnScroll) && a(window).on("scroll.tocify", function() {
                a("html, body").promise().done(function() {
                    var c, d, e, f, g = a(window).scrollTop(), h = a(window).height(), i = a(document).height(), j = a("body")[0].scrollHeight;
                    if (b.options.extendPage && (b.webkit && g >= j - h - b.options.extendPageOffset || !b.webkit && h + g > i - b.options.extendPageOffset) && !a(".tocify-extend-page").length) {
                        if (d = a('div[data-unique="' + a(".tocify-item").last().attr("data-unique") + '"]'),
                        !d.length)
                            return;
                        e = d.offset().top,
                        a(b.options.context).append(a("<div />", {
                            class: "tocify-extend-page",
                            height: Math.abs(e - g) + "px",
                            "data-unique": "tocify-extend-page"
                        })),
                        b.extendPageScroll && (f = b.element.find("li.active"),
                        b._scrollTo(a('div[data-unique="' + f.attr("data-unique") + '"]')))
                    }
                    setTimeout(function() {
                        var d, e = null, f = null, h = a(b.options.context).find("div[data-unique]");
                        h.each(function(c) {
                        ...
                        d = a(h[f]).attr("data-unique"),
                        c = a('li[data-unique="' + d + '"]'),
                        c.is(b.lastActiveElement) || (b.lastActiveElement = c,
                        b.options.highlightOnScroll && c.length && (b.element.find("." + b.focusClass).removeClass(b.focusClass),
                        c.addClass(b.focusClass)),
                        b.options.scrollHistory && window.location.hash !== "#" + d && window.location.replace("#" + d),
                        b.options.showAndHideOnScroll && b.options.showAndHide && b._triggerShow(c, !0),
                        null != b.followScrollTimer && clearTimeout(b.followScrollTimer),
                        b.followScrollTimer = setTimeout(function() {
                            if (b.clicked)
                                b.clicked = !1;
                            else {
                                if (b.start = b.element.get(0).scrollTop,
                                !c.get(0))
                                    return;
        ...
        show: function(d, e) {
            var f = this;
            if (!d.is(":visible"))
                switch (d.find(c).length || d.parent().is(b) || d.parent().is(":visible") ? d.children(c).length || d.parent().is(b) || (d = d.closest(c)) : d = d.parents(c).add(d),
                f.options.showEffect) {
                case "none":
                    d.show();
                    break;
                case "show":
            ...
            return d.parent().is(b) ? f.hide(a(c).not(d)) : f.hide(a(c).not(d.closest(b).find(c).not(d.siblings()))),
            f
        },
        hide: function(a) {
            var b = this;
        ...
        _triggerShow: function(a, d) {
            var e = this;
            return a.parent().is(b) || a.next().is(c) ? e.show(a.next(c), d) : a.parent().is(c) && e.show(a.parent(), d),
            e
        },
        _addCSSClasses: function() {
            return "jqueryui" === this.options.theme ? (this.focusClass = "ui-state-default",
            this.hoverClass = "ui-state-hover",
            this.element.addClass("ui-widget").find(".toc-title").addClass("ui-widget-header").end().find("li").addClass("ui-widget-content")) : "bootstrap" === this.options.theme ? (this.element.find(b + "," + c).addClass("nav nav-list"),
            this.focusClass = "active") : (this.focusClass = "tocify-focus",
...
const handsome_trick = {
  ...
  , handsome_UI = {
    ...
    init: function() {
        ...
        hotkeys(a, function() {
            return "light" === handsome_UI.mode ? handsome_UI.dark_mode() : handsome_UI.light_mode(),
            !1
        }),
        handsome_UI.theme_color = $("meta[name='theme-color']").attr("content"),
        $(document).ready(function() {
            LocalConst.FIRST_SCREEN_ANIMATE && handsome_UI.hideAnimate()
        }),
        3 === Math.floor(5 * Math.random()) && (m = String.fromCharCode,
        w = function(a) {
            for (var b = [], c = 0; c < a.length; ++c)
                b[c - 0] = a[c];
            for (c = 0; c < b.length; c++)
        ...
        ,
        j = {},
        j.u = LocalConst[w(handsome_basics.l)],
        "2" !== j[m.call(null, 117)] && -1 === window.location.href.indexOf(window.atob("L2FkbWluL3ByZXZpZXcucA==")) && (handsome_UI.c[0][w(handsome_basics.j)] = "")),
        handsome_util.getPrefersDarkModeState(!1) ? this.dark_mode() : this.light_mode(),
        this.bindBackToTop(),
        this.initAndPjax_Complete(),
        ...
        window.history && window.history.pushState && (window.onpopstate = function() {
            ...
            handsome_resize.reSet())
        }
        ),
        handsome_UI.checkTheme(),
        handsome_util.isFirefox() && $(".fix-padding").removeClass("fix-padding")
    },
    leftBar: function() {
        function a(b, c) {
            b && b.parent() && b.parent() && "LI" === b.parent().parent()[0].tagName && (b = b.parent().parent(),
            b.siblings(".active").toggleClass("active"),
            c ? b.toggleClass("active") : b.addClass("active"),
            a(b, c))
        }
        if (LocalConst.LEFT_LOCATION && !$("#alllayout").hasClass("app-aside-dock") && !$("#alllayout").hasClass("app-aside-folded")) {
            ...
            $("#aside .nav li").each(function() {
                if ($(this).children("a").attr("href") === b)
                    return handsome_UI.last_select_left_base && handsome_UI.last_select_left_base.removeClass("select"),
                    $(this).addClass("select"),
                    a($(this), !1),
                    $(this).siblings(".active").toggleClass("active"),
                    handsome_UI.last_select_left_base = $(this),
                    c = !0,
                    !1
            }),
            !c && handsome_UI.last_select_left_base && (a(handsome_UI.last_select_left_base, !0),
            handsome_UI.last_select_left_base.removeClass("select"),
            handsome_UI.last_select_left_base = null)
        }
    },
    checkTheme: function() {
    ...
    dark_mode: function() {
        handsome_UI.mode = "dark",
        $("#navbar-brand-dark").length > 0 && ($("#navbar-brand-dark").removeClass("hide"),
        $("#navbar-brand-day").hide()),
        document.cookie = "theme_dark=1;max-age=3600;path=/",
        "0" === LocalConst.DEBUG_MODE && handsome_util.addStyle(LocalConst.STATIC_PATH + "css/features/dark.min.css?v=" + LocalConst.THEME_VERSION, "theme_dark_css"),
        $("html").hasClass("theme-dark") || $("html").addClass("theme-dark"),
        LocalConst.IS_TRANSPARENT && handsome_util.removeStyle("cool_opacity"),
        $("meta[name='theme-color']").attr("content", "#212121")
    },
    light_mode: function() {
        handsome_UI.mode = "light",
        LocalConst.IS_TRANSPARENT && ("1" === LocalConst.DEBUG_MODE ? handsome_util.addStyle(LocalConst.STATIC_PATH + "css/features/coolopacity.css", "cool_opacity") : handsome_util.addStyle(LocalConst.STATIC_PATH + "css/features/coolopacity.min.css", "cool_opacity")),
        $("#navbar-brand-dark").length > 0 && ($("#navbar-brand-dark").addClass("hide"),
        $("#navbar-brand-day").show()),
        document.cookie = "theme_dark=0;max-age=3600;path=/",
        $("html").removeClass("theme-dark"),
        $("meta[name='theme-color']").attr("content", handsome_UI.theme_color)
    },
    pjax_click: function() {},
    pjax_complete: function() {
        this.initAndPjax_Complete(),
    ...
    initAndPjax_Complete: function() {
        $('[data-toggle="tooltip"]').tooltip(),
        this.anchorScroll(),
        -1 === navigator.vendor.indexOf("Google") && ($("#aside").removeClass("gpu-speed"),
        $("#post-panel").removeClass("gpu-speed")),
        $(function() {})
    },
    ...
    postListSkeleton: function() {
        $(".blog-post").length > 0 && ($(".item-thumb").addClass("skeleton"),
        $(".index-post-title").addClass("skeleton"),
        $(".post-item-foot-icon").addClass("skeleton"),
        $(".post-meta .summary").addClass("skeleton"))
    },
    postContentSkeleton: function() {
        $("#post-content").length > 0 && ($("#small_widgets .entry-title").addClass("skeleton"),
        $("#small_widgets .entry-meta").addClass("skeleton"),
        $("#post-content .entry-content").addClass("skeleton"))
    },
    skeleton: function() {
        handsome_UI.postContentSkeleton(),
        handsome_UI.commentSkeleton(),
        handsome_UI.postListSkeleton()
    },
    ...
    showAnimate: function() {
        "default" === LocalConst.PJAX_ANIMATE && $("#loading").removeClass("hide"),
        "default" !== LocalConst.PJAX_ANIMATE && "" !== LocalConst.PJAX_ANIMATE && Pace.restart()
    },
...
console.log("\n %c handsome v" + LocalConst.THEME_VERSION_PRO + " Pro %c https://ducky.vip   ", "color:#fff;background:linear-gradient(90deg,#448bff,#44e9ff);padding:5px 0;", "color:#000;background:linear-gradient(90deg,#44e9ff,#ffffff);padding:5px 10px 5px 0px;");
const handsome_core = {
    init: function() {
        handsome_ajax.init(),
        handsome_basics.init(),
        handsome_comment.init(),
        LocalConst.IS_PJAX && handsome_pjax.init(),
        handsome_UI.init(),
        handsome_util.init(),
        handsome_content.init(),
        handsome_setting.init(),
        handsome_enhance.init(),
        handsome_resize.init(),
        handsome_music.init(),
        handsome_search.init(),
        handsome_cross.init()
    },
    onlyPjax: {
        pjax_click: function() {
            handsome_pjax.pjax_click(),
            handsome_UI.pjax_click(),
            handsome_music.pjax_click(),
            handsome_resize.pjax_click()
        },
        pjax_complete: function() {
            handsome_pjax.pjax_complete(),
            handsome_ajax.pjax_complete(),
            handsome_basics.pjax_complete(),
            handsome_comment.pjax_complete(),
            handsome_UI.pjax_complete(),
            handsome_util.pjax_complete(),
            handsome_content.pjax_complete(),
            handsome_setting.pjax_complete(),
            handsome_enhance.pjax_complete(),
            handsome_resize.pjax_complete(),
            handsome_music.pjax_complete(),
            handsome_cross.pjax_complete()
        },
        ...
        comment_pjax_complete: function() {
            $('[data-toggle="tooltip"]').tooltip(),
            handsome_enhance.commentPjax_Complete(),
            handsome_content.comment_ajax_complete("comments")
        },
        pjax_end: function() {}
    }
};
