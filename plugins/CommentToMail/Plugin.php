<?php
/**
 * 评论邮件提醒插件--梦繁星魔改--handsome专用
 *
 * @package CommentToMail
 * @author Byends, DEFE
 * @version 2.0.1
 * @link http://blog.emoao.com
...
class CommentToMail_Plugin implements Typecho_Plugin_Interface
{
    ...
    {        
        ...
        $cfg = array(
            ...
            'ip'        => $comment->ip,
            'title'     => $comment->title,
            'imgurl'    => getAuthorImg($comment->mail),
            'text'      => parseBiaoQing($comment->text),
    ...
    public static function curl($url)
    {
        ...
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 10);  // 连接等待时间
        curl_setopt($ch, CURLOPT_TIMEOUT, 10);         // curl允许执行时间
